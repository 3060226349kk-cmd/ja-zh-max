...
语域脏度: [1-5] | 策略: [准确优先/平衡/流畅优先], [敬语降维策略], [和製漢語两步法]
-->
```
...
**判定方法：** 阅读原文前 500–1000 字 → 判断是否存在贬称/骂詈/性描写/暴力描写 → 按密度和强度赋 1–5。不确定时宁高不低（高可下调，低导致整本译脏度不足）。
...
#### ★ jp-zh-max 增强：术语表语域强制查阅（阶段 2 准入）
...
- 与「和製漢語两步法」配合——两步法提供流程，本方法论提供每步的决策准则
...
- 日文与中文脏字体系不对等时，译者需在中文语域中自行定位对等层级
...
**2. 译文修改分层框架**（预载方法论）— 在三层修改（篇章→句子→词汇）基础上增加第四层：
- 篇章层：段落间衔接、信息流
- 句子层：语法结构、长修饰分割
- 词汇层：和製漢語陷阱、选词精准度
- **修辞层（★ 新增）**：辞格处理、敬语降维、文体审美一致性

四层修改的产出直接替代原有的三层修改清单。修改记录在阶段 8A 清理时删除。
...
1. **剥夺版本标记** — 删除所有内部工序标记：
   - 标准版记：`[v1]`、`[v2·S]`、`[v2·W]`、`[v2·A]`、`[v2·∅]`、`[v3]`、`[v3·Q✓]`
...
脚本自动完成上述三项操作并输出统计。**脚本跑完 ≠ 门禁通过**——还需执行以下四项 grep 验证（四项均须通过）：
...
# 验证 4：项目地址默认必须存在
grep -c 'github.com/3060226349kk-cmd/ja-zh-max' "<名称> 翻译(日中对照).md" && echo '✅ 项目地址已添加' || echo '❌ 项目地址缺失！'
```

> ⚠ **验证 1–4 任一项失败 = 门禁未通过。** 退回修复后重跑脚本 + 重验，直到四项全部通过。

#### 步骤 8B — 派生交付物

> ⚠ **准入：** 步骤 8A 四项 grep 验证全部通过。
